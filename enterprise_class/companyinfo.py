...
def industry_classification_sql():
    ...
    # connection = pymysql.connect(host='rm-8vbif49m6k7l651e5fo.mysql.zhangbei.rds.aliyuncs.com', user='root',
    #                              password='Liqin1988', db='python', charset='utf8',
    #                              cursorclass=pymysql.cursors.DictCursor)
    connection = pymysql.connect(host='10.8.32.125', user='root', password='Admin123!', db='engineering-brain', charset='utf8',
    ...
    sql = "SELECT `scope` FROM companyinfo_enterprise_base_info"  # 国务院为table名
    cursor.execute(sql)
    result = cursor.fetchall()  # 获取标题
    for item in result:  # item为字典格式
        title = item.get('scope')  # 获取每一条政策的标题
        processed_title = title_processing(title, replace_words, stop_word)  # 获取处理后的政策标题
        industry_label1, industry_label2 = get_industry_label(processed_title)  # 获取机器标签
        sql = "UPDATE companyinfo_enterprise_base_info SET industry_classify = '%s',function_classify = '%s' WHERE `scope` = '%s'" % (
