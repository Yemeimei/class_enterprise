...
def industry_classification_sql():
    ...
    sql = "SELECT title FROM topic_info_zhaotoubiao_old"  # 国务院为table名
    cursor.execute(sql)
    ...
    for item in result:  # item为字典格式
        ...
        industry_label1, industry_label2 = get_industry_label(processed_title)  # 获取机器标签
        sql = "UPDATE topic_info_zhaotoubiao_old SET industry_classify = '%s',function_classify = '%s' WHERE title = '%s'" % (
